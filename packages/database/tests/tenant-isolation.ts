...
async function main() {
  ...
  try {
    ...
    // Seed test fixtures through the privileged migration/admin connection.
    // We explicitly set the tenant context here because FORCE RLS applies even
    // to the table owner, and membership INSERT policies intentionally require
    // a matching app.user_id/app.org_id context.
    await admin.query('BEGIN');

    const a = (
      await admin.query(`INSERT INTO users(email, name) VALUES ($1,'A') RETURNING id`, [
        `a-${suffix}@test.local`,
      ])
    ).rows[0].id;
    const b = (
      await admin.query(`INSERT INTO users(email, name) VALUES ($1,'B') RETURNING id`, [
        `b-${suffix}@test.local`,
      ])
    ).rows[0].id;
    const orgA = (
      await admin.query(`INSERT INTO organizations(name, slug) VALUES ('Org A',$1) RETURNING id`, [
        `org-a-${suffix}`,
      ])
    ).rows[0].id;
    const orgB = (
      await admin.query(`INSERT INTO organizations(name, slug) VALUES ('Org B',$1) RETURNING id`, [
        `org-b-${suffix}`,
      ])
    ).rows[0].id;

    await context(admin, a, orgA);
    await admin.query(
      `INSERT INTO organization_members(organization_id,user_id,role) VALUES ($1,$2,'OWNER')`,
      [orgA, a],
    );
    const wsA = (
      await admin.query(`INSERT INTO workspaces(organization_id,name,slug) VALUES ($1,'A',$2) RETURNING id`, [
        orgA,
        `a-${suffix}`,
      ])
    ).rows[0].id;
    await admin.query(
      `INSERT INTO projects(organization_id,workspace_id,name,slug,created_by) VALUES ($1,$2,'A',$3,$4)`,
      [orgA, wsA, `a-${suffix}`, a],
    );

    await context(admin, b, orgB);
    await admin.query(
      `INSERT INTO organization_members(organization_id,user_id,role) VALUES ($1,$2,'OWNER')`,
      [orgB, b],
    );
    const wsB = (
      await admin.query(`INSERT INTO workspaces(organization_id,name,slug) VALUES ($1,'B',$2) RETURNING id`, [
        orgB,
        `b-${suffix}`,
      ])
    ).rows[0].id;
    await admin.query(
      `INSERT INTO projects(organization_id,workspace_id,name,slug,created_by) VALUES ($1,$2,'B',$3,$4)`,
      [orgB, wsB, `b-${suffix}`, b],
    );

    await admin.query('COMMIT');

    // Verify tenant isolation through the actual runtime identity.
    ...
    const mutation = await runtime.query(
      `UPDATE projects SET name='blocked' WHERE organization_id=$1 RETURNING id`,
      [orgB],
    );
